  ...
  */
  ...
  function handleDelete(id: string | string[]) {
    ...
    const deleteData = async () => {
      ...
      //await fetch(`/api/doingdoit/board/deleteOneComment?id=${id}`);

      await fetch(`/api/doingdoit/board/deleteComment?id=${id}`);



      const res = await fetch(`/api/doingdoit/board/getCommentCountByBoardId?_boardId=${boardId}&_q=${searchTerm}`);
      ...
      const res2 = await fetch(`/api/doingdoit/board/getCommentsByBoardId?_boardId=${boardId}&_limit=${countPerPage}&_page=${currentPage}&_sort=${sortConfig.key}&_order=${sortConfig.direction}&_q=${searchTerm}`);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      if (session?.user?.email == 'admin@orangex.center') {
  


        const res = await fetch(`/api/doingdoit/board/getCommentsByBoardId?_boardId=${boardId}&_limit=${countPerPage}&_page=${currentPage}&_sort=${sortConfig.key}&_order=${sortConfig.direction}&_q=${searchTerm}`);
        ...
        const res = await fetch(`/api/doingdoit/board/getAllCommentByEmail?email=${session?.user?.email}&_limit=${countPerPage}&_page=${currentPage}&_sort=${sortConfig.key}&_order=${sortConfig.direction}&_q=${searchTerm}`);
  ...
  ,[ searchTerm, sortConfig, currentPage, countPerPage, session?.user?.email  ]);
