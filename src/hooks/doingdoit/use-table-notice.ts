  ...
  */
  ...
  const fetchData = async (
    ...
    if (session?.user?.email.includes('@orangex.center')) {
      ...
      ///const res = await fetch(`/api/doingdoit/notice/getAllByEmail?email=${session?.user?.email}&_limit=${countPerPage}&_page=${currentPage}&_sort=${sortConfig.key}&_order=${sortConfig.direction}&_q=${searchTerm}`);
  ...
  */
  ...
  function handleDelete(id: string | string[]) {
    ...
    const deleteData = async () => {
      ...
      await fetch(`/api/doingdoit/notice/deleteOne?id=${id}`);
