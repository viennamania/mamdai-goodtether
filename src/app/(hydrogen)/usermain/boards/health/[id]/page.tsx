  ...
  */
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      ///const res = await fetch('/api/doingdoit/healthinfo/getBoardById?_id=' + id);

      const res = await fetch(`/api/doingdoit/healthinfo/getOne?id=${id}`);
  ...
  const like = () => {
    ...
    // update my like list
    const res = fetch(`/api/doingdoit/healthinfo/like?_id=${id}&_userId=${userData?.id}&_userNickname=${userData?.nickname}&_userAvatar=${userData?.avatar}&_userEmail=${userData?.email}`);
  ...
  return (
    ...
    <div className="relative bg-white w-full overflow-hidden flex flex-col items-center justify-start text-left text-xs text-dark font-menu-off">
      ...
      self-stretch flex flex-col items-center justify-start">
        ...
        <div className=" 
        self-stretch xl:bg-gray-50 flex flex-col items-center justify-start py-5 xl:py-10 ">
