...
export default function BoardEditPage({ 
  ...
  console.log('BoardEditPage id: ', id);
  ...
  */
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
        ...
        const res = await fetch(`/api/doingdoit/board/getBoardById?_id=${id}&_userId=${userId}`);
        const json = await res?.json();
        ...
        console.log('BoardEditPage data.data: ', data.data);
        ...
        if (data.data) {
          ...
          setBoardContent(data.data.content);
  ...
  console.log('BoardEditPage tagList: ', tagList);



  console.log('BoardEditPage oard?.tags: ', board?.tags);
  ...
  const update  = async (
    ...
    try {
      ...
      //////const res = await fetch(`/api/doingdoit/feed/updateFeedById?_id=${_id}&_feedTitle=${feedTitle}&_feedContent=${feedContent}&_name=${userName}&_avatar=${userAvatar}&_nickname=${userNickname}`);
      ...
      const res = await fetch(`/api/doingdoit/board/update`, {
        method: 'POST',
        headers: {
      ...
      if (res.status === 200) {
        ...
        if (json === undefined) {
          ...
          if (data)  {
            ...
            window.location.href = `/usermain/boards/${id}`;
  ...
  */
  ...
  const onSubmit: SubmitHandler<BoardFormTypes> = (data) => {
    ...
    update(title as string, content as string, tagList as any, hiddenYn as string);

    */

    update(boardTitle as string, boardContent as string, tagList as any, 'N');
