  ...
  */
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      //const res = await fetch(`/api/doingdoit/feed/getStatisticsTotalByEmail?_email=${session?.user?.email}`);

      ///const res = await fetch(`/api/doingdoit/feed/getStatisticsMonthByEmail?_email=${session?.user?.email}`);


      //////const res = await fetch(`/api/doingdoit/feed/getStatistics?_email=${session?.user?.email}&_mealDate=2023-12-19`);


      const dateYearMonth = moment(mealDate).format('YYYY-MM');
      
      
      //const res = await fetch(`/api/doingdoit/feed/getStatisticsMonthByEmail?_email=${session?.user?.email}&_mealDate=${mealDate}`);

      const res = await fetch(`/api/doingdoit/feed/getStatisticsMonthByEmail?_email=${session?.user?.email}&_mealDate=${dateYearMonth}`);
...
useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      //const res = await fetch(`/api/doingdoit/feed/getFeedbackStatisticsByEmail?_email=${session?.user?.email}`);


      //const res = await fetch(`/api/doingdoit/feed/getFeedbackStatisticsMonthByEmail?_email=${session?.user?.email}`);


      // mealDateStart is this month first date
      // mealDateEnd is this month last date

      const mealDateStart = moment(mealDate).startOf('month').format('YYYY-MM-DD');
      const mealDateEnd = moment(mealDate).endOf('month').format('YYYY-MM-DD');

      //const res = await fetch(`/api/doingdoit/feed/getFeedbackStatisticsWeekByEmail?_email=${session?.user?.email}`);

      const res = await fetch(`/api/doingdoit/feed/getFeedbackStatisticsMonthByEmail?_email=${session?.user?.email}&_mealDateStart=${
        mealDateStart
      }&_mealDateEnd=${  
        mealDateEnd
      }
                ...
                <div className="flex flex-row justify-center items-center gap-8 mb-10  ">
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      router.push(`/usermain/feeds/statistics/monthly/${moment(mealDate).subtract(1, 'month').format('YYYY-MM')}`);
                  ...
                  <DatePicker
                    ...
                    onChange={(date: Date) => {
                      ...
                      //changeDate(date);
                      router.push(`/usermain/feeds/statistics/monthly/${moment(date).format('YYYY-MM')}`);
                    ...
                    ///monthClassName="w-12"
                    // month cell width is 12rem

                    // change font size of month cell
                    
                    

                    ///popper-monthClassName=" m-10 p-5 w-20 text-xl font-extrabold text-dark  "
                    ...
                    inputProps={{
                      ...
                      // when media is xl, then font-size is 1.25rem, else 1rem
                      fontSize: { xl: '1.25rem', lg: '1rem', md: '1rem', sm: '1rem', xs: '1rem' },
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      router.push(`/usermain/feeds/statistics/monthly/${moment(mealDate).add(1, 'month').format('YYYY-MM')}`);
