...
//import { getColumns } from '@/app/shared-doingdoit/feed/stats-columns';
  ...
  */
  ...
  useEffect(() => {
    ...
    const getFeedbackWriter = async () => {
      ...
      //const response = await fetch('/api/doingdoit/feed/getAllFeedbackWriter');
  ...
  const exportToCSV = async (fileName: string) => {
    ...
    setIsExporting(true);
      ...
      const res = await fetch('/api/doingdoit/feed/getStatsAllDownload', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
    ...
    posts?.data?.data.map((item : any  , index : number) => {
      ...
      */
      ...
      formattedData = [
        ...
        {'일자': '2024-03-07', '김회피': 1, '김구매주문': 1, '주진우': 1, '장연수': 1, '정수민': 1, '하하하': 1, '고민정': 1, '손지명': 1, '이정훈': 1, '손오공': 1, '김정은': 1, '정은아': 1, '이은정': 1, 'GoodTether': 1}
        {'일자': '2024-03-06', '김회피': 1, '김구매주문': 1, '주진우': 1, '장연수': 1, '정수민': 1, '하하하': 1, '고민정': 1, '손지명': 1, '이정훈': 1, '손오공': 1, '김정은': 1, '정은아': 1, '이은정': 1, 'GoodTether': 1}
        {'일자': '2024-03-05', '김회피': 1, '김구매주문': 1, '주진우': 1, '장연수': 1, '정수민': 1, '하하하': 1, '고민정': 1, '손지명': 1, '이정훈': 1, '손오공': 1, '김정은': 1, '정은아': 1, '이은정': 1, 'GoodTether': 1}
      ]
      */
